...
or describe static hardware identity, should be EntityCategory.DIAGNOSTIC: a
disk's SMART warning and model, a pool's health problem, and a Docker
container's update-available flag. Everything else (system info, pool
usage/free space, disk power status, service status, pool maintenance
operations, LXC/Docker container state) is a regular sensor.
"""
...
DIAGNOSTIC_ENTITY_SUFFIXES = ("_smart_warning", "_problem", "_update_available", "_model")
...
async def test_only_smart_warning_and_problem_are_diagnostic(
    ...
    """Every mos entity is DIAGNOSTIC iff its id ends in _smart_warning, _problem, _update_available, or _model."""
