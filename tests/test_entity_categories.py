...
Only entities that genuinely serve diagnosing a problem should be
EntityCategory.DIAGNOSTIC: a disk's SMART warning and a pool's health
problem. Everything else (system info, pool usage/free space, disk power
status, service status, pool maintenance operations) is a regular sensor.
"""
...
DIAGNOSTIC_ENTITY_SUFFIXES = ("_smart_warning", "_problem")
...
async def test_only_smart_warning_and_problem_are_diagnostic(
    ...
    """Every mos entity is DIAGNOSTIC iff its id ends in _smart_warning or _problem."""
